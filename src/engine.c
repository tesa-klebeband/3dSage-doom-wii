...
#define SW (SCREEN_WIDTH / 2)
#define SH (SCREEN_HEIGHT / 2)
...
typedef struct
{
    uint8_t w, s, a, d; // move up, down, left, rigth
    uint8_t sl, sr;     // strafe left, right
    uint8_t m;          // move up, down, look up, down
} keys;
keys K;
...
void load()
{
    FILE *fp = fopen("/apps/3dSage-doom/level.h", "r");
    if (fp == NULL)
    {
        VIDEO_ClearBuffer(rgb(255, 0, 0));
        while(1)
        {
            checkWiimote();
        }
    ...
    for (x = x1; x < x2; x++)
    {
        ...
        if (frontBack == 0)
        {
            ...
            for (y = y1; y < y2; y++)
            {
                ...
                int r = Textures[wt].name[pixel] - W[w].shade / 2;
                ...
                if (b < 0)
                    b = 0;
                VIDEO_SetPixel(x, SH - y, rgb(r, g, b));
                vt += vt_step;
            } // normal wall
            ht += ht_step;
        }
        ...
        if (frontBack == 1)
        {
            ...
            for (y = ys; y < ye; y++)
            {
                ...
                float rx = fx * M.sin[P.a] - fy * M.cos[P.a] + (P.y / 118.0 * tile);
                float ry = fx * M.cos[P.a] + fy * M.sin[P.a] - (P.x / 118.0 * tile);
                if (rx < 0)
                    rx = -rx + 1;
                if (ry < 0)
                    ry = -ry + 1;
                ...
                if (b < 0)
                    b = 0;
                VIDEO_SetPixel(x2 + xo, SH - (y + yo), rgb(r, g, b));
            }
        }
    }
}
...
void draw3D()
{
    int x, s, w, wx[4], wy[4], wz[4], frontBack, cycles;
...
void checkWiimote()
{
    ...
    if (wpad_data->btns_h & WPAD_BUTTON_RIGHT)
        K.w = 1;
    if (wpad_data->btns_h & WPAD_BUTTON_LEFT)
        K.s = 1;
    if (wpad_data->btns_h & WPAD_BUTTON_UP)
        K.a = 1;
    if (wpad_data->btns_h & WPAD_BUTTON_DOWN)
        K.d = 1;
    if (wpad_data->btns_h & WPAD_BUTTON_1)
        K.sr = 1;
    if (wpad_data->btns_h & WPAD_BUTTON_2)
        K.sl = 1;
    if (wpad_data->btns_h & WPAD_BUTTON_B)
        K.m = 1;
    if (wpad_data->btns_h & WPAD_BUTTON_HOME)
        exit(0);
}

void display()
{
    VIDEO_ClearBuffer(rgb(20, 20, 20));
    movePlayer();
    draw3D();
    checkWiimote();
}
